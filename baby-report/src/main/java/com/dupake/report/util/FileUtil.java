package com.dupake.report.util;
...
import com.dupake.report.exception.ServiceException;
...
public class FileUtil {
    ...
    public static boolean checkExt(String fileName, String[] exts) {
        if (StringUtils.isBlank(fileName))
            throw new ServiceException("待校验文件名不能为空");
        if (exts == null || exts.length == 0)
            throw new ServiceException("文件后缀名数组不能为空");
